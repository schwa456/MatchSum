...
class My_WandbLogger(WandbLogger):
    ...
    def _scan_and_log_checkpoints(self, checkpoint_callback):
        # Except the artifact, it is equal to the original
        checkpoints = {
        ...
        for t, p, s in checkpoints:
            metadata = (
                {
                    "score": s,
                    "original_file_name": Path(p).name,
                    "ModelCheckpoint": {
            ...
            if self._save_artifact:
                ...
                aliases = ["lastest", "best"] if p == checkpoint_callback.best_model_path else ["lastest"]
